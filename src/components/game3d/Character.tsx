...
export default function Character({ onPositionChange, onSprintChange, mobileInput, mobileJump, mobileSprint }: CharacterProps) {
  ...
  useFrame((_, delta) => {
    ...
    if (moveDir.length() > 0) {
      ...
      // Collision check against landmarks, lake, and trees
      const colliders = [
      ...
      // Add trees as colliders
      for (const tree of TREE_POSITIONS) {
        colliders.push({ x: tree.x, z: tree.z, r: 1.2 });
      }
    ...
    if (onPositionChange) {
      onPositionChange([charPos.x, charPos.y, charPos.z], groupRef.current.rotation.y);
    }
    if (onSprintChange) {
